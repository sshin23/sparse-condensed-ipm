...
implementations—MadNLP.jl and ExaModels.jl—running on NVIDIA GPUs
...
a computational framework for solving large-scale AC OPF problems on
...
primarily due to the need for numerical pivoting, which requires irregular
...
ExaModels.jl. The solution of the KKT system is performed using the
...
solution framework that can run entirely on GPU, with all the
...
This section covers three essential background topics: numerical
optimization, AD, and GPU computing.\subsection{Numerical Optimization}\label{sec:numopt}
...
tools, MadNLP.jl and ExaModels.jl, are implemented in the Julia Language,
...
algebraic modeling interface in ExaModels.jl enforces the users to
specify the model equations in an {\tt Generator} data type in
...
array is given. This process drastically saves computational costs
...
\eqref{eqn:first}. In the sequel, we shall drop the superscript
...
DenseCondensedKKTSystem}, etc, whose data are stored either on the host or device memory. For the implementation of the condensed-space IPM
