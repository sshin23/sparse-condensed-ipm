...
over 20 times speedup by running on the GPU. Compared with general AD
...
as relative constraint violation is used as convergence criteria.}
This
...
portion of the total solution time. {\color{blue} This can be observed from Figure
\ref{fig:speedup}; linear algebra is the only bottleneck in
large-scale instances.  For large instances, GPUs can only achieve
approximately 5x speedup.  These speedup ratios can vary depending on
the instances.  For instance, for case24464\_goc, the GPU linear
solver performance was close to that of the CPU solver.  The
investigation of under which circumstances cuSOLVER is more effective
